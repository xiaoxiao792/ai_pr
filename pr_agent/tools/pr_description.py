...
def remove_checklist_section(description: str) -> str:
    if not description:
        return description

    checklist_header = re.compile(r"(?im)^###\s+\*\*Check List\*\*\s*$")
    next_section = re.compile(r"(?im)^(?:___\s*$|###\s+)")
    match = checklist_header.search(description)
    if not match:
        return description

    next_match = next_section.search(description, match.end())
    end = next_match.start() if next_match else len(description)
    return (description[:match.start()] + description[end:]).strip()
...
class PRDescription:
    def __init__(self, pr_url: str, args: list = None,
        ...
        self.vars = {
            ...
            "description": remove_checklist_section(self.git_provider.get_pr_description(full=False)),
