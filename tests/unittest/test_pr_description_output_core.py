...
from pr_agent.tools.pr_description import PRDescription, remove_checklist_section
...
# _prepare_data
# ---------------------------------------------------------------------------
class TestPromptDescriptionCleanup:
    def test_remove_checklist_section_keeps_other_description_sections(self):
...
"""
        ...
        cleaned = remove_checklist_section(description)

        assert "### **Check List**" not in cleaned
        assert "<Change Type>" not in cleaned
        assert "Intro text" in cleaned
        assert "### **Background**" in cleaned
        assert "Keep this useful context." in cleaned
