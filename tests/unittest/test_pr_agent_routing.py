...
@pytest.mark.asyncio
async def test_handle_request_plus_review_uses_combined_reviewer_without_changing_review(monkeypatch):
    review_calls = []
    plus_review_calls = []

    assert pr_agent_module.command2class["review"] is pr_agent_module.PRReviewer
    assert pr_agent_module.command2class["plus_review"] is pr_agent_module.PlusPRReviewer

    class FakeReviewer:
        def __init__(self, pr_url, ai_handler=None, args=None):
            review_calls.append((pr_url, ai_handler, args))

        async def run(self):
            pass

    class FakePlusPRReviewer:
        def __init__(self, pr_url, ai_handler=None, args=None):
            plus_review_calls.append((pr_url, ai_handler, args))
    ...
    _patch_request_dependencies(monkeypatch)
    monkeypatch.setattr(pr_agent_module, "PRReviewer", FakeReviewer)
    monkeypatch.setattr(pr_agent_module, "PlusPRReviewer", FakePlusPRReviewer)
    monkeypatch.setitem(pr_agent_module.command2class, "review", FakeReviewer)
    monkeypatch.setitem(pr_agent_module.command2class, "plus_review", FakePlusPRReviewer)

    handled_review = await pr_agent_module.PRAgent(ai_handler="fake-ai")._handle_request(
        "https://example/pr/1", "/review"
    )
    handled_plus_review = await pr_agent_module.PRAgent(ai_handler="fake-ai")._handle_request(
        "https://example/pr/1", "/plus_review"
    )

    assert handled_review is True
    assert handled_plus_review is True
    assert review_calls == [("https://example/pr/1", "fake-ai", [])]
    assert plus_review_calls == [("https://example/pr/1", "fake-ai", [])]
