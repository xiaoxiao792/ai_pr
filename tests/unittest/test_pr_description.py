...
class TestPRDescriptionCore:
    def test_describe_prompt_requires_commit_messages_as_change_context(self):
        prompt_path = Path(__file__).parents[2] / "pr_agent" / "settings" / "pr_description_prompts.toml"
        prompt = prompt_path.read_text()

        assert "Treat the commit messages as important change intent/context" in prompt
        assert "do not mechanically copy or list commit messages" in prompt
        assert "If commit\n  messages conflict with the diff, trust the diff" in prompt
